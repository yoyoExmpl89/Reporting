                ...
                <div class="box-body">
                    ...
                    <div class="form-group" id="SalesSelect">
                        ...
                        <select class="form-control" id="Sales">
							...
							<!-- if bukan kacab dan admin hanya bisa liat id sendiri -->
							<!-- jika kacab dan admin bisa liat semua di list dia -->
...
<script type="text/javascript">
    $( document ).ready(function() {
        ...
        $("#formkirim").submit(function(e){
			e.preventDefault();
            getAum($("#datepicker").val(),$('#SalesSelect option:selected').val(),$('#SelectQty option:selected').val());
    ...
    function getAum(startDate,salesid,qty){
		showModal();
        var url = "http://172.16.3.126:8080/testla/rest-points/asmgService/QueryAumSalesNasabah/"+startDate+"/"+salesid+"/"+qty;
        $.ajax({
            url: url,
            dataType: 'json'
        }).done(function( msg ) {
            ...
            $('#headTable').append('<tr><th>ASDATE</th><th>INV_ACCT_NAME</th><th>SELLER_NAME </th><th>PRODTYPE </th><th>branchname </th><th>amt_bal </th></tr>');

            $("#myModal").modal("hide");
            $.each(JSON.parse(msg), function(key, val) {
                $('#bodyTable').append('<tr><td>'+val.asdate+'</td><td>'+val.INV_ACCT_NAME+'</td><td>'+val.SELLER_NAME+'</td><td>'+val.PRODTYPE+'</td><td>'+val.branchname+'</td><td>'+val.amt_bal+'</td></tr>');
            });   
			$("#submit").prop("disabled","disabled");
        });        
    }
