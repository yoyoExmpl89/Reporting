...
<script type="text/javascript">
    ...
    function getClientRisk(){
        showModal();
        var url = "http://172.16.3.126:8080/testla/rest-points/s21Service/selectQueryAuditClientRisk/";
        $.ajax({
            url: url,
            dataType: 'json'
        }).done(function( msg ) {
            ...
            $('#headTable').append('<tr><th>cifid</th><th>cifinvestortype</th><th>ciftype </th><th>cifinvestorriskcode </th><th>riskdesc </th><th>businesstypedesc </th><th>CChardesc </th></tr>');

            $("#myModal").modal("hide");
            $.each(JSON.parse(msg), function(key, val) {
                $('#bodyTable').append('<tr><td>'+val.cifid+'</td><td>'+val.cifinvestortype+'</td><td>'+val.ciftype+'</td><td>'+val.cifinvestorriskcode+'</td><td>'+val.riskdesc+'</td><td>'+val.businesstypedesc+'</td><td>'+val.CChardesc+'</td></tr>');
            });    
        });        
    }
	function showModal() {
