...
<script type="text/javascript">
    ...
    function get26(stDate,enDate){
        showModal();
        var url = "http://172.16.3.126:8080/testla/rest-points/asmgService/djpTin26/"+stDate+"/"+enDate;
        $.ajax({
            url: url,
            dataType: 'json'
        }).done(function( msg ) {
            ...
            $('#headTable').append('<tr><th>mi_id</th><th>DocRefID_AccountReport</th><th>PaymentType </th><th>CurrencyCodePayment </th><th>PaymentAmnt</th></tr>');

            $("#myModal").modal("hide");
            $.each(JSON.parse(msg), function(key, val) {
                $('#bodyTable').append('<tr><td>'+val.mi_id+'</td><td>'+val.DocRefID_AccountReport+'</td><td>'+val.PaymentType+'</td><td>'+val.CurrencyCodePayment+'</td><td>'+val.PaymentAmnt+'</td></tr>');
            });    
        });        
    }
	function showModal() {
