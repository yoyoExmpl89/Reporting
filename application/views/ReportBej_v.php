...
<script type="text/javascript">
    $( document ).ready(function() {
        ...
        $("#formkirim").submit(function(e){
			e.preventDefault();
            getTinaDataBej(startDate,endDate,startDate,endDate);
    ...
    function getTinaDataBej(stDate,enDate,stDate,enDate){
         showModal();
        var url = "http://172.16.3.126:8080/testla/rest-points/s21Service/selectTina/"+stDate+"/"+enDate+"/"+stDate+"/"+enDate;
        $.ajax({
            url: url,
            dataType: 'json'
        }).done(function( msg ) {
            ...
            $('#headTable').append('<tr><th>PERIODE</th><th>CHANNEL</th><th>BUY_VAL </th><th>SELL_vAL </th></tr>');

            $("#myModal").modal("hide");
            $.each(JSON.parse(msg), function(key, val) {
                $('#bodyTable').append('<tr><td>'+val.PERIODE+'</td><td>'+val.CHANNEL+'</td><td>'+val.BUY_VAL+'</td><td>'+val.SELL_vAL+'</td></tr>');
