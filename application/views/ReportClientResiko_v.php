...
<script type="text/javascript">
    $( document ).ready(function() {
        ...
        $("#formkirim").submit(function(e){
			e.preventDefault();
            getClientResiko($("#datepicker").val(),$("#datepicker").val());
    ...
    function getClientResiko(stDate,enDate){
		showModal();
        var url = "http://172.16.3.126:8080/testla/rest-points/s21Service/QueryClientResiko/"+stDate+"/"+enDate;
        $.ajax({
            url: url,
            dataType: 'json'
        }).done(function( msg ) {
            ...
            $('#headTable').append('<tr><th>CIFID</th><th>ClientID</th><th>ClientName </th><th>CIFTYPE </th><th>CLIENT_IA </th><th>CLIENTTYPE </th><th>AKTIF_YN </th></tr>');

            $("#myModal").modal("hide");
            $.each(JSON.parse(msg), function(key, val) {
                $('#bodyTable').append('<tr><td>'+val.CIFID+'</td><td>'+val.ClientID+'</td><td>'+val.ClientName+'</td><td>'+val.CIFTYPE+'</td><td>'+val.CLIENT_IA+'</td><td>'+val.CLIENTTYPE+'</td><td>'+val.AKTIF_YN+'</td></tr>');
            });    
        });        
    }
	function showModal() {
