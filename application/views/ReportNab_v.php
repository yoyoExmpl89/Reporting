...
<script type="text/javascript">
    $( document ).ready(function() {
		...
		$.ajax({
			url : "<?php echo site_url('ReportNab/get_data_sales');?>",
			method : "POST",
			async : true,
			dataType : 'json',
			success: function(data){
                //alert(data);
				var html = '';
				for(i=0;i<data.sellercd.length;i++){
					html += '<option value='+data.sellercd[i].Sellercd+'>'+data.sellercd[i].SellerName+'</option>';
				}
				$('#Sales').append(html);
    ...
    function getNab(sellercd){
        showModal();
        var url = "http://172.16.3.126:8080/testla/rest-points/asmgService/datanab/"+sellercd;
        $.ajax({
            url: url,
            dataType: 'json'
        }).done(function( msg ) {
