...
<script type="text/javascript">
    $( document ).ready(function() {
        ...
        $.ajax({
			url : "<?php echo site_url('DjpTinOrganisation/get_prod');?>",
			method : "POST",
			async : true,
			dataType : 'json',
        ...
        // action goes here!!
		 ...
		 $("#formkirim").submit(function(e){
			e.preventDefault();
            //alert($('[name="MIID"]').val()); 
            getAccRepInOrganisation($("#datepicker").val(),$('[name="MIID"]').val());
    ...
    function getAccRepInOrganisation(startDate,inputMI){
		showModal();
       // alert(startDate);
        
        var url = "http://172.16.3.126:8080/testla/rest-points/asmgService/djpTinOrganisation/"+startDate+"/"+inputMI;
        $.ajax({
            url: url,
            dataType: 'json'
        }).done(function( msg ) {
            ...
            $('#headTable').append('<tr><th>DocRefID_AccountReport</th><th>TIN</th><th>IssuedBy </th><th>TINType </th></tr>');

            $("#myModal").modal("hide");
            $.each(JSON.parse(msg), function(key, val) {
                $('#bodyTable').append('<tr><td>'+val.DocRefID_AccountReport+'</td><td>'+val.TIN+'</td><td>'+val.IssuedBy+'</td><td>'+val.TINType+'</td></tr>');
            });    
        });        
    }
