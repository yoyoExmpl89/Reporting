...
<script type="text/javascript">
    ...
    function getCbestNasabah(){
        $("#myModal").modal({backdrop: 'static', keyboard: false, show : true});
        var url = "http://172.16.3.126:8080/testla/rest-points/s21Service/QueryCBESTNasabahAll";
        $.ajax({
            url: url,
            dataType: 'json'
        }).done(function( msg ) {
            //alert( "Data Saved: " + msg );
            $('#headTable').append('<tr><th>ClientName</th><th>KSEISubAccountNo</th></tr>');

            $("#myModal").modal("hide");
            $.each(JSON.parse(msg), function(key, val) {
                $('#bodyTable').append('<tr><td>'+val.ClientName+'</td><td>'+val.KSEISubAccountNo+'</td></tr>');
    ...
    function getCbestNasabahBranch(inputcb){
        showModal();
        var url = "http://172.16.3.126:8080/testla/rest-points/s21Service/QueryCBESTNasabah/"+inputcb;
        $.ajax({
            url: url,
            dataType: 'json'
        }).done(function( msg ) {
